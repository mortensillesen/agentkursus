...
(function () {
  ...
  function vurder(tempo, kontrol) {
    ...
    r.data = kontrol === 1 ? "risiko: ingen har besluttet, hvilke repos og servere der er tilladt" : (tempo === 3 ? "ok, hvis listen over tilladte servere er kort og vedligeholdt" : "ok: listen over tilladte servere er lille");
    r.godkendelse = kontrol === 3 ? "ok: pull requests og CI-porte før merge" : (tempo === 1 ? "acceptabelt: piloten kan nøjes med manuel review" : "risiko: ændringer lander uden port");
    r.kompetence = tempo === 1 ? "ok: få personer, der kan læres op i kriterier og transkripter" : (tempo === 2 ? "pres: en afdeling kræver kursus og en, der kan hjælpe" : "risiko: kompetencespringet rammer alle på én gang");
  ...
  holder.appendChild(el("div", { class: "sim" }, [
    ...
    liste,
    el("p", { class: "note-lille", text: "Læg mærke til, at kompetence og tillid ikke kan løses med kontrol. De løses med tempo. Det er derfor, dokumentationen anbefaler en pilotgruppe og en baseline før udvidelse." })
  ]));
  opdater();
})();
