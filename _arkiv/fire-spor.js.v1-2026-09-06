...
(function () {
  ...
  var SPOR = [
    { navn: "Spor 1: test_soe.py", fil: "tests/test_soe.py", sek: 35 },
    { navn: "Spor 2: test_flere_kolli.py", fil: "tests/test_flere_kolli.py", sek: 45 },
    { navn: "Spor 3: test_tillaeg.py", fil: "tests/test_tillaeg.py", sek: 30 },
    { navn: "Spor 4: test_cli.py", fil: "tests/test_cli.py", sek: 40 }
  ];
  var kollision = false;
  var wrap = el("div");
  function tegn() {
    ...
    if (kollision) { spor[2].fil = "tests/test_pris.py"; spor[3].fil = "tests/test_pris.py"; spor[2].navn = "Spor 3: tillæg i test_pris.py"; spor[3].navn = "Spor 4: CLI i test_pris.py"; }
    ...
    opsum.appendChild(el("tbody", null, [
      el("tr", null, [el("td", { text: "Sekventielt, ét spor ad gangen" }), el("td", { text: sekv + " s agenttid, alt i ét vindue" })]),
      el("tr", null, [el("td", { text: "Parallelt, fire spor" }), el("td", { text: maks + " s plus opstart og samling, cirka fire gange så mange tokens" })]),
      el("tr", null, [el("td", { text: "Resultat" }), el("td", { text: kollision ? "Spor 3 og 4 skrev i samme fil. Den sidste, der skrev, vandt. Halvdelen af testene forsvandt, og suiten er grøn, fordi det manglende ikke testes." : "Fire filer, ingen overlap. Hovedagenten kører suiten til sidst og samler." })])
    ]));
  ...
  holder.appendChild(el("div", { class: "sim" }, [
    ...
    el("p", { class: "note-lille", text: "Tiderne er illustrative. I kursets egen måling var den parallelle kørsel langsommere end den sekventielle, fordi opstart og samling kostede mere, end sporene sparede. Mål selv i øvelsen." })
  ]));
  tegn();
})();
