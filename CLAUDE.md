...
Morten kan ikke kode og skal ikke lære det. Kurset lærer ham at bestille arbejde hos Claude Code, følge med og bedømme resultatet ud fra bevis. Det styrer alt indhold:

* Teknisk stof, der beholdes: det, Morten selv taster. `claude`, kommandoer i sessionen (`/clear`, `/compact`, `/context`, `/usage`, `/mcp`, `/exit`), nulstil-kommandoen, at køre beregneren og testene, at kopiere en færdig konfigurationsfil ind.
* Teknisk stof, der fjernes: kodens inderside. Funktionsnavne, Python-ord, YAML-nøgler, protokoldetaljer, exit-koder. Filnavne kun i prompts til agenten og i kommandoer.
* Kommandoer, Morten taster, er én linje uden pipes, jq, løkker eller `python3 -c`. Alt svært bliver en bestilling til agenten. Git er agentens arbejde, bortset fra nulstil-kommandoen og `git config core.hooksPath`.
* Hvert modul åbner Kernen med et billede fra spedition eller undervisning, før det tekniske.
* Stilguiden med oversættelser og faste billeder ligger i `noter/stilguide-ikke-kodende.md`.
...
* Version 1.2.0, 2026-09-07: 15 infografikker, én for hele begrebet på forsiden og én først i Kernen i hvert modul. Kilden er `infografik/byg.py` (SVG), PDF'erne er importeret i Canva som redigerbare designs, og kurset viser Canvas PNG-eksport fra `assets/img/infografik/`. Ny bloktype `billede` med `fil`, `alt` og `tekst`. Design-id'er og arbejdsgang i `noter/infografik.md`.
* Version 1.1.0, 2026-09-06: alle 14 moduler og simulationer omskrevet til en læser, der ikke koder. Forsidens tekst flyttet fra index.html til `kursus.json` under `forside`. Modul 01 fik en ny opgave (transportformen kurer) og er målt igen. Sandboxens `start/modul-01` har nu samme tynde CLAUDE.md som 04 til 10. Nulstil-kommandoen i alle sandbox-moduler lægger nu løse ændringer til side med `git stash push -u`, før den skifter branch, så en øvelse aldrig efterlader noget, der blokerer den næste.
