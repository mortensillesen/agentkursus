...
(function () {
  ...
  function tegn() {
    kol.textContent = "";
    var generel = [
      { t: "Læser skillens seks trin ind i kontekst" },
      { t: "Kalder liste_zoner, læser svaret" },
      { t: "Kalder hent_takst for fly og soe, læser svarene" },
      aendret ? { t: "Svaret hedder nu \"128,00 kr/kg (fly, zone 5)\". Agenten læser det nye format og fortsætter.", k: "ok" } : { t: "Skriver taksterne i JSON" },
      { t: "Hooken kører testene. Subagenten regner efter." },
      { t: "Cirka 8 turns, 70 til 80 sekunder, 0,3 til 0,5 dollar", k: aendret ? "ok" : "" }
    ];
    var destillat = [
      { t: "Sender initialize og tre tools/call over stdio" },
      { t: "Parser tallet ud af \"zone 5 (Oversoeisk), fly: 128.00 DKK per kg\"" },
      aendret ? { t: "Svaret hedder nu \"128,00 kr/kg (fly, zone 5)\". Parseren finder intet tal. Scriptet stopper med en fejl.", k: "fejl" } : { t: "Skriver taksterne i JSON og kører testene" },
      aendret ? { t: "Du opdager det, fordi exit-koden er 1, ikke fordi resultatet er forkert. Rettelsen er tre linjer.", k: "fejl" } : { t: "1 sekund, 0 modelkald, 0 dollar", k: "ok" },
  ...
  var knap = el("button", { type: "button", text: "Lad serveren ændre sit svarformat" });
  knap.addEventListener("click", function () { aendret = !aendret; knap.textContent = aendret ? "Nulstil svarformatet" : "Lad serveren ændre sit svarformat"; tegn(); });
